...
# Return exact (previous, following) index pairs used by the evaluation plan.
passage_transitions <- function(indices, plan) {
  ...
  list(previous = indices[positions], following = indices[positions + 1L])
}
...
bigram_baseline <- function(training_indices, heldout_indices, plan, vocab_size) {
  ...
  -mean(log((counts[cbind(pairs$previous, pairs$following)] + 1) /
